...
int companyPublicKey;
uint8_t hashedEncryptedData[32];
uint8_t signatureEncryptedData[65];

Web3 *web3;
Crypto *crypto;    
Contract energyContract = Contract(web3, CONTRACT_ADDRESS);

// Converts an uint8 array to a hex encoded string
...
void sendInstantPower(Contract energyContract, unsigned int instantPower) {
    ...
    endTime = micros();
    Serial.printf("Time needed to Set Transaction Parameters (excluding smart contract function and parameters): %d\n", endTime - startTime);
    startTime = micros();
    ...
    retCode = mbedtls_pk_parse_public_key(&pk, companyPK, sizeof(companyPK));
    ...
    const unsigned char toEncrypt = instantPower;
    size_t olenp = 0;
    string dataToSend;
    ...
    if((retCode = mbedtls_pk_encrypt(
        ...
        dataToSend = "0x" + uint8ArrayToHexString(encrypted, olenp);
        endTime = micros();
        Serial.printf("Time Needed to Encrypt Data with Company PK: %d\n", endTime - startTime);
        startTime = micros();
        Serial.printf("\nEncrypted Data: %s\n", dataToSend.c_str());
    }

    // Hash and Sign crypted power data 
    crypto->Keccak256(encrypted, 32, hashedEncryptedData);
    endTime = micros();
    Serial.printf("Time needed to Hash Encrypted Data: %d\n", endTime - startTime);
    startTime = micros();
    crypto->Sign(hashedEncryptedData, signatureEncryptedData);
    endTime = micros();
    Serial.printf("Time needed to Sign Encrypted Data: %d\n", endTime - startTime);
    startTime = micros();

    /*
    endTime = micros();
    Serial.printf("Time needed to do Signing Operations (only getting previusly created signature and hash): %d\n", endTime - startTime);
    startTime = micros();
    */
    ...
    string strHash = "0x" + uint8ArrayToHexString(hashedEncryptedData, 32);
    ...
    energyContract.SendTransaction(nonceVal, gasPriceVal, gasLimitVal, &caddress, &valueStr, &dataStr);
    endTime = micros();
    Serial.printf("Time needed to Send Transaction: %d\n", endTime - startTime);
...
void setup() {
    ...
    eepromPrivateKey = EEPROM.readString(EEPROM_PK_ADDR);
    if (eepromPrivateKey.isEmpty()) {
        EEPROM.writeString(EEPROM_PK_ADDR, String(PRIVATE_KEY));
        EEPROM.commit();
    }
    ...
    // Init Entropy Context and CRT Function
    ...
    endTime = micros();
    Serial.printf("Time needed to do all setup operations, first time only (connect to wifi, establish connection to the blockchain, setup cryptographic modules): %d\n", endTime - startTime);
    startTime = micros();
...
void loop() {
    ...
    if(txCounter == txLimit) {
        ...
        if (currentMillis - previousMillis >= interval) {
            ...
            startTime = micros();
            sendInstantPower(energyContract, 3);
            // Serial.printf("\nTX COUNT: %d\n", txCounter);
