...
Web3::Web3(long long networkId, const char* ip_address, unsigned short address_port) {
    mem = new BYTE[sizeof(WiFiClientSecure)];
    ...
    // path = strcat(strcat(strdup(host), ":"), "9545");
    path = "/";
}
...
string Web3::getJson(const string* json) {
    TagReader reader;
    string parseVal = reader.getTag(json, "result");
    return parseVal;
}
